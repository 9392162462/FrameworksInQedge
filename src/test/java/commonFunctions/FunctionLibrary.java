...
public class FunctionLibrary {
	...
	public static void customerTable() throws Throwable
	{
		...
		//method for writing customer number into Note pad
		public static void capturepurchases(String LocatorType,String LocatorValue) throws Throwable
		{
			String purchasesnum ="";
			if(LocatorType.equalsIgnoreCase("id"))
			{
				purchasesnum = driver.findElement(By.id(LocatorValue)).getAttribute("value");
			}
			if(LocatorType.equalsIgnoreCase("xpath"))
			{
				purchasesnum = driver.findElement(By.xpath(LocatorValue)).getAttribute("value");
			}
			if(LocatorType.equalsIgnoreCase("name"))
			{
				purchasesnum = driver.findElement(By.name(LocatorValue)).getAttribute("value");
			}
			//create note pad and write customer number
			FileWriter fw = new FileWriter("./CaptureData/purchasesnum.txt");
			BufferedWriter bw = new BufferedWriter(fw);
			bw.write(purchasesnum);
			bw.flush();
			bw.close();
		}
		//method for customer table
		...
		{
			//read customer number from Note pad
			FileReader fn = new FileReader("./CaptureData/purchasesnum.txt");
			BufferedReader bn = new BufferedReader(fn);
			String Exp_Data = bn.readLine();
			WebElement searchTextbox= driver.findElement(By.xpath(conpro.getProperty("search-textbox")));
			if(!searchTextbox.isDisplayed())
				driver.findElement(By.xpath(conpro.getProperty("search-panel"))).click();
			searchTextbox.clear();
			...
			Reporter.log(Exp_Data+"     "+Act_Data,true);
			try {
				Assert.assertEquals(Act_Data, Exp_Data, "Purchases Number is Not Matching");
			} catch (AssertionError	a) {
				System.out.println(a.getMessage());
			}
	}
